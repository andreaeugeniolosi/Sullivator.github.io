...
function renderCanvas2() {
  if (drawing2) {
    ...
    lastPos2 = mousePos2;    
    if (lfo) {
      o3.frequency.value=lastPos2.y/4 -lastPos2.x/4;
    }
    else {
...
var synth_jazz = new Tone.PolySynth(6, Tone.Synth, {
    oscillator : {
          type : "sine"
      },
      envelope : {
...
var x_jazz = ["C3", "A2", "D3", "G2"];
var y_jazz = ["E3", "C3", "F2", "B2"];
var z_jazz = ["G2", "E3", "A2", "D3"];
var w_jazz = ["B4", "G4", "C4", "F4"];
bpm=document.getElementById("tempo").value;

function progression_jazz(i_jazz) {
    synth_jazz.triggerAttackRelease([x_jazz[i_jazz], y_jazz[i_jazz], z_jazz[i_jazz], w_jazz[i_jazz]], period1/1000*2);
}

var x_jazz_sad = ["C3", "Ab2", "D3", "G2"];
var y_jazz_sad = ["Eb3", "C3", "F2", "B2"];
var z_jazz_sad = ["G2", "Eb3", "Ab2", "D3"];
var w_jazz_sad = ["Bb4", "G4", "C4", "F4"];

function progression_jazz_sad(i_jazz) {
    synth_jazz.triggerAttackRelease([x_jazz_sad[i_jazz], y_jazz_sad[i_jazz], z_jazz_sad[i_jazz], w_jazz_sad[i_jazz]], period1/1000*2);
}

var i_jazz=0;

function play_progression_jazz() {
    period=60/bpm*1000;
    period_prog=period*2;
    progression_jazz(i_jazz)
    refreshID_jazz=setTimeout(play_progression_jazz,period_prog)
    i_jazz++
    if (i_jazz>3) {
        i_jazz=0;
    }
...
function play_progression_jazz_sad() {
    period=60/bpm*1000;
    period_prog=period*2;
    progression_jazz_sad(i_jazz)
    refreshID_jazz_sad=setTimeout(play_progression_jazz_sad,period_prog)
    i_jazz++
    if (i_jazz>3) {
        i_jazz=0;
    }
...
function stop_prog_jazz(){
  clearTimeout(refreshID_jazz);
  synth_jazz.triggerAttackRelease([x_jazz[i_jazz-1], y_jazz[i_jazz-1], z_jazz[i_jazz-1], w_jazz[i_jazz-1]], 0.00001 );
}

function stop_prog_jazz_sad(){
  clearTimeout(refreshID_jazz_sad);
  synth_jazz.triggerAttackRelease([x_jazz_sad[i_jazz-1], y_jazz_sad[i_jazz-1], z_jazz_sad[i_jazz-1], w_jazz_sad[i_jazz-1]], 0.00001 );
}
